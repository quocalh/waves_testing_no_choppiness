...
#define WAVES_NUMBER 100
#define MAX_AMPLITUDE 2.5
#define MAX_SPEED 1.5
#define WIND_SPEED 0.7
#define choppy_coef 0.7
...
{   
    ...
    vec3 paramatric_position = vec3(in_Position.x, 0, in_Position.z);
    normal = vec3(0, 0.0, 0);
    ...
    for (int i = 0; i < WAVES_NUMBER; i++)   // ★ fixed bounds
    {
        ...
        paramatric_position += vec3(
            ((dir.x / w) * (displacement * computed_cos)),
            (A * computed_sin),
            ((dir.y / w) * (displacement * computed_cos))
        );

        normal += vec3(
            -dir.x * A * w * cos(p),
            1 + choppy_coef * A * w * computed_sin - sin(p + choppy_coef * Dz_coef) * (1 - choppy_coef * A * w * computed_sin),
            -dir.y * A * w * cos(p)
        );
    }
...
{   
    ...
    vec3 specular = specular_strength * pow(specular_lit, 80) * sky_color;
    ...
    // glFragColor += vec4(vec3(
    //     pow((clamp((length(unnormalized_normal) - 130) / 60, 0, 1)), 1.3)
    //     ), 1) * 1.8;
